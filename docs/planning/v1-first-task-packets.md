...
- **A.1 — schema-typed service catalog** ⏳ (declared closed task-type enum per `CapacityOffer`, one #135 MCP tool per service, not a generic completion proxy — "the biggest structural mitigation"). *Ripples `CapacityOffer`/`match_offer`; whether it's a hard requirement (reject offers without a task type) or opt-in convention is open-question 3 — build as opt-in, flag the enforcement decision.*
