...
- **BC2 — require-at-setup wiring ⏳:** a per-channel `BillingPolicy { None | Require{payee, terms_hash, min_amount} }` consulted at channel-join (mirrors staple option-A); the joining agent presents a `BillingCommitment`, the peer calls `satisfies`. Unit-gatable.
