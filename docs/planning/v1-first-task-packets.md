...
- **#147-L3 — consumption proof** ⏳ (per-`CapacityKind` attestation of the provider response / local-inference result). **Req-2 DECIDED (scimbe 2026-07-23): highest-correctness-enforcement scheme = co-signed usage receipts** — provider **and** consumer co-sign a token-count receipt; the co-signed receipt is the settlement trigger (symmetric, hard to forge, one shape for both kinds). Build as a signed-claim type (`UsageReceipt`, dual signature over a domain-separated preimage) in the `CapacityOffer` family.
