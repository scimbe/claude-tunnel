...
- **#144-②-registry** ⏳: a searchable registry — `GET /registry/agents?role=&skill=` over the control-plane, backed by agents self-registering their published card URL. Additive control-plane REST + a store table (per the #44 additive-migration rule). Decompose fully once ① is wired.
