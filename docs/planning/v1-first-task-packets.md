...
- **N136.4 — cross-NAT punch proof** ⏳ — reclassified (scimbe steer 2026-07-21: *"docker way first, then real sink/source"*). The punch can't be a **cargo** test (no NAT on loopback), but it **is** verifiable in the repo's Docker-only-emulation model via a network-namespace 2-NAT lab — no real hosts needed for the repeatable proof; the live plane test is the follow. Decomposed:
  ...
  - **N-wire — wire the DCUtR path into the live join** ⏳: `run_channel_session_upgradable_dcutr` is currently orphaned (only called from tests → `#[allow(dead_code)]`), so even on real NATs two members relay forever and never punch. Give the agent a runnable `CT_CHANNEL_CIRCUIT_RELAY` mode that constructs the libp2p relay-client swarm and dispatches to it for relay-only members, consulting `should_attempt_direct_upgrade`. Removes the dead_code; the config/decision layer is hermetic (frozen test), the swarm construction runs in N-rig-2.
