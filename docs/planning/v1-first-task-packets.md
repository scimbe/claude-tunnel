...
- **A.1 — schema-typed service catalog** ⏳ **(enforcement DECIDED scimbe 2026-07-24: opt-in convention)** — add a declared task-type enum to `CapacityOffer` as an **opt-in** capability; a generic (catalog-less) offer stays valid (no rejection). Provider registers one #135 MCP tool per declared service. Ripples `CapacityOffer` (additive field, mirror the A.4/A.5 zero-sentinel pattern to avoid breaking callers) + optionally `match_offer` (match on task-type when present). Decision-free now — buildable as a bounded slice.
