...
- **#144-②-registry** 🟡 **(store ✅):** `storage::SqliteAgentDirectory` (`agent_cards` table, distinct from the tunnels `SqliteRegistry`): agents `register(holder_pubkey, card_url, role_tags, skill_ids, now)` (upsert keyed by holder key — re-registering updates, no dupes), and peers `search(role, skill)` by **exact token** (not substring; role+skill compose AND; both `None` → all). Directory is DISCOVERY only — it points at the verifiable `card_url`; a searcher fetches `/.well-known/agent-card.json` and re-checks the holder signature (never trusts the registry). Frozen test `agent_directory_upserts_and_searches_by_exact_role_and_skill`. Gate green, 0 warnings. **Remaining ②:** the REST surface — `POST /registry/agents` (self-register, OIDC-bearer-gated per #87) + `GET /registry/agents?role=&skill=` (public search) in `service.rs`, mounting the store. Bounded follow.
