...
- **#147-L2 — bid/ask/match** ⏳ (auction messages ride #135 L2 MCP-over-channel; bid with floor, seller ask, match). **Req-4 DECIDED (scimbe 2026-07-23): escrow at match** — on a match, lock the consumer's funds and release to the provider on the L3 consumption proof (≈ a cap, not a true loss). Needs a settlement **hold/escrow** primitive on the L4 chain (lock → release-on-proof / refund-on-timeout) before the matcher wires it.
