...
- **#107-optimize-follow** ⏳ (pure half ✅): edge latency probes feed `plan_network_overlay`'s `latency`; then the controller
  ...
  **REQUIRED precondition (#113, preventive):** before `add_shortcuts`/`plan_network_overlay` is wired into ANY
  ...
  - **#107-ui-suggest ⏳ (surface the adaptive optimizer):** a "suggest best overlay" endpoint running `plan_network_overlay_bounded` over the topology's agents + measured/`link_cost_from_probes` costs, returning the suggested links — makes the already-built adaptive approximation (best physical-network usage) usable from the editor. Hermetic (pure optimizer over stored state).
