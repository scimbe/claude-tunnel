...
- **#144-①-wiring** ⏳: mount `agent_card_response` on the browser-mode origin path so `ct-agent onboard --mode browser` serves the card automatically — the "subdomain+cert+card in one command" ask. Needs the browser-mode HTTP intercept in the agent's serve/proxy path (`serve.rs`) — a bounded integration follow.
