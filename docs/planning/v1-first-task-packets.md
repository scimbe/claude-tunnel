...
- **L1 — `AgentCard` (identity/discovery)** 🟡 *unit-gatable when ripe.* A holder-signed card `{holder_pubkey, role_tags, skills[], cells[], channels[], issued_at, expires_at, signature}` — another object in the `ct_common::channel` signed-claim family (domain `ct-agent-card-v1`), same verify discipline as BC1. Published via the collective-memory vault / a registry issue (no HTTP `/.well-known` — we're relay-only). Bounded packet: type + `signing_bytes`/`is_valid` + frozen test (mirrors BC1). **Cleanly buildable; awaits central's go on scope (role-tag vocabulary, skill schema).**
