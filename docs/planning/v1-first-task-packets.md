...
- **N136.4 — cross-NAT punch proof** ⏳ — reclassified (scimbe steer 2026-07-21: *"docker way first, then real sink/source"*). The punch can't be a **cargo** test (no NAT on loopback), but it **is** verifiable in the repo's Docker-only-emulation model via a network-namespace 2-NAT lab — no real hosts needed for the repeatable proof; the live plane test is the follow. Decomposed:
  ...
  - **N-rig-2b — libp2p DCUtR punch smoke** ⏳: run the actual libp2p relay + two DCUtR clients across the three namespaces and assert a REAL direct upgrade (`dcutr` `DirectConnectionUpgradeSucceeded` → direct path between the two NAT gateways' reflexive addresses → payload flows direct while the relay drops). **GUARDRAIL (found N-rig-2):** `build_relay_swarm` is deliberately private + *test-only* (an unguarded relay; invariant #3's `C-membership-gate` must land before any live/public relay). So the punch harness must be a **test-only, cargo-feature-gated binary** (e.g. `--features nat-lab`), **never** a production `ct-agent channel relay` subcommand — shipping an open relay would be a footgun. This binary exposes the p2p relay/DCUtR helpers under `#[cfg(any(test, feature = "nat-lab"))]` and `nat-lab.sh` orchestrates the 3 roles across the namespaces.
