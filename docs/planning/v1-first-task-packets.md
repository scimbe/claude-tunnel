...
- **#107-optimize-follow** ⏳ (pure half ✅): edge latency probes feed `plan_network_overlay`'s `latency`; then the controller
  ...
  **REQUIRED precondition (#113, preventive):** before `add_shortcuts`/`plan_network_overlay` is wired into ANY
  ...
  - **#107-ui-compose 🟡 (draw-edge ✅):** the editor now lets the owner GRAPHICALLY compose the overlay — a **"Connect"** bar tool (`<button id="link">`, `aria-pressed`) that, when armed (`svg[data-linkmode='1']`), turns clicking two agent nodes into a new link POSTed to the existing owner `POST …/edges` endpoint; on success the edge is drawn **live** (`insertAdjacentHTML` in SVG context — deliberately NOT `createElementNS('http://…/svg')`, whose namespace literal would break the zero-external-assets/CSP-safe invariant), with a self-link / duplicate-link guard and per-action `#msg` feedback. `linkPick`/`addEdgeEl`/`edgeExists` are inline (CSP-safe) JS; drag is suppressed while the tool is armed. Frozen test `topology_editor_supports_click_to_connect_compose` (Connect tool present + un-armed, link-mode gate, POST-to-`/edges` wiring, live edge without a namespace literal, still zero external assets) + the existing `topology_editor_page_is_a_self_contained_draggable_node_graph` unchanged (edge count still 3 — the compose JS carries no `class="edge"` literal). Gate green, 0 warnings. **Remaining #107-ui-compose:** interactive **add-agent** (`POST …/agents`) and **edge removal** (needs a new owner `DELETE …/edges` REST leg) — render-hermetic, live-apply.
