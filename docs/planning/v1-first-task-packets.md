...
- **L2 — MCP-over-Agent-Fabric-channel (capability exposure)** ⏳ *integration, sequenced.* MCP JSON-RPC as the application bytes of an open #133 channel. Sink's transport reality-check (verified): the gate is **persistent-session vs. pipe-and-exit** (re-paying a handshake per call guts it) + "tools return references, not blobs" (~14 KB/s, #76). So this sequences **after #104's direct-upgrade** (now landed) and needs a persistent-session primitive first. Not a single bounded packet — a mini-epic.
