...
- **N136.2 — DCUtR establish adapter** ⏳ *(unit-gatable over loopback):* `establish_direct_via_dcutr(stream, role, keys) = establish_direct_session(split(stream), …)` — turns the DCUtR duplex into the pump-ready `(TransportState, read, write)`. Frozen test over the loopback DCUtR harness (analog of `establish_direct_session`'s test).
