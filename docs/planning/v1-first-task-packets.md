...
- **N136.1 — production DCUtR client primitive** ⏳ *(the real work; refactor, unit-harness-testable):* extract a reusable `dcutr_dial_via_relay(relay_addr, peer, …) -> P2pDuplex` (and the reservation/listen side) from the in-process `connected_dcutr_stream_pair` test harness, so a real agent can reserve on a **real** relay + dial a peer through it + attempt the DCUtR upgrade, yielding an `AsyncRead+AsyncWrite` stream. The existing harness becomes a thin wrapper that stands up an in-process relay and calls it (so it stays gate-green over loopback — the punch is trivial on loopback, but the *code path* is exercised). **This is where the effort is.**
