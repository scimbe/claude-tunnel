...
- **#147-L2 — bid/ask/match** ⏳ (auction messages ride #135 L2 MCP-over-channel; bid with floor, seller ask, match). *Needs the req-4 escrow decision.*
- **#147-L3 — consumption proof** ⏳ (per-`CapacityKind` co-signed attestation of the provider response / local-inference result — the hard req-2 scheme). *Needs the discussion round; do NOT design ahead.*
- **#147-L4 — dedicated settlement chain** 🟡 *(maintainer go 2026-07-23: "Ja mach").* A **real** chain (req 3), distinct from the symbolic billing family.
  ...
  - **L4.2 — consensus (fork-choice / voting)** ⏳ (leader election + fork resolution across writers; `append`+`accept_block` give the produce/receive primitives it sequences — no dedicated Rust BFT lib in-tree, so this is a real build, its flavor [raft vs byzantine] a maintainer/design call).
