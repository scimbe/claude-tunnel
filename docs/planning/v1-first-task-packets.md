...
- **N136.1 — production DCUtR client primitive** 🟡 *(dialer ✅ `dc2a5b6`; reserve/accept side ⏳):* extracted `pub(crate) dcutr_dial_via_relay(client, peer_via_relay, target_peer) -> P2pDuplex` (the B-side dial + substream-open) out of the in-process `connected_dcutr_stream_pair` harness — the harness now calls it (behavior-preserving; both DCUtR tests pass through it 5/5). `DcutrRelayClientBehaviour` is `pub(crate)` so channel_run (N136.3) can call it. **Remaining ⏳:** the sibling **reserve/accept** primitive (the relay-only peer's A-side: reserve on the relay, listen, return the inbound stream) — a two-phase API (reserve→signal→accept) that's fiddlier to extract than the dialer.
...
- **N136.3 — wire into the upgrade** ⏳ *(integration):* in `run_channel_session_upgradable`, when the peer is relay-only (a relay-circuit endpoint, not a `SocketAddr`), inject the DCUtR establish closure instead of `dial_peer_direct`; the Offer carries the relay-circuit multiaddr (+ holder-signed coordinates, reusing `SignedCoordinateRecord`/Kademlia already in p2p.rs). Loopback end-to-end test (analog of `upgradable_session_over_quinn_…`).
