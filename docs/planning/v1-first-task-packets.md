...
- **Provisioning — idempotency.** Our `POST /enroll/issue-batch` mints N tokens but is **not idempotent**: a retried request mints N *more* tokens (duplicate identities on a network blip). Marq's fix (idempotency key + desired agent record → dedupe) is a real gap → **#145-batch-idempotent** (planned): accept an `idempotency_key`, return the same token set on replay, don't re-mint. Bounded + testable next slice.
