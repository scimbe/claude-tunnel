...
- **#147-L4 — dedicated settlement chain** 🟡 *(maintainer go 2026-07-23: "Ja mach").* A **real** chain (req 3), distinct from the symbolic billing family.
  ...
  - **L4.2 — consensus (fork-choice / voting)** ⏳ **flavor DECIDED (scimbe 2026-07-23: "fast and fault-tolerant") = Raft single-leader** (leader appends, followers replicate via `accept_block`; term + leader election; crash-fault-tolerant f<n/2 — the *fast* option, not the heavyweight Byzantine BFT build). Bounded first slices: (i) **L4.2-gossip-idempotency ✅** (`622c1fc`) — `accept_block` treats a re-delivered already-held block as an idempotent no-op (dup = Ok, fork = reject); flavor-agnostic, shipped. (ii) **L4.2-term/vote** ⏳ — a `Term`/`Vote` signed-claim + `elect_leader` primitive (reuse the ascending-holder tie-break style of `elect_superpeers`). (iii) **L4.2-leader-append** ⏳ — only the elected leader `append`s; followers reject a non-leader block for the term. Build incrementally, one slice per fire.
