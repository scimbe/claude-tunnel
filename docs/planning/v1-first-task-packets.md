...
- **C-membership-gate ⏳:** the invariant-#3 enforcement — a superpeer relays a circuit **only** for a channel it is a grant-member of (the `C-circuit-relay-transport` relay is deliberately unguarded; this slice adds the admission check that makes a live relay safe to run). **Blocks any live/public relay deployment.**
