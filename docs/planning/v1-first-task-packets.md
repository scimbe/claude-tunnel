...
- **#144-①-wiring** 🟡 **(mountable router ✅):** `ct-agent::well_known::agent_card_router(card) -> axum::Router` serves the card at the well-known path (404 elsewhere), frozen-tested over HTTP (`oneshot`: `200`/json/verifiable-after-round-trip + `/nope`→404). **Key finding:** browser mode (`CT_AGENT_MODE=browser`, #23) is **raw TLS passthrough** — the agent forwards opaque bytes and never sees the request path, so the card is served **AT THE ORIGIN**, not intercepted by the agent. The router is direction-agnostic (origin-helper OR onboard-launched). **Remaining ①-wiring:** bind the router to a TLS listener on the agent's subdomain as a first-class `ct-agent onboard --mode browser` output (a standalone origin helper: listener + the subdomain cert) — the final "one command" integration; a small design point (does onboard launch the server, or emit a runnable helper) worth a central nod before wiring.
