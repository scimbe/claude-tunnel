...
| `CT_CP_EDGE_ADMIN_TOKEN` | control plane | **opt-in** 64-hex shared admin token gating the machine/operator writer endpoints (`/enroll/issue`, `/registry/register`, `/accounts/open`, `/payment/intent`, `/billing/issue`, `/bootstrap/mint`) (#87/#90); unset ⇒ those routes stay open (dev/back-compat). Reads, customer `/me/*`, and the portal are unaffected. The front-door overlay sets it from `CT_EDGE_ADMIN_TOKEN`, so the edge and control plane share one value |
