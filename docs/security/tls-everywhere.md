...
| External ↔ Control-plane API | **HTTPS terminated at the ingress**; plaintext HTTP is redirected to HTTPS | `control-plane-ingress.yaml` (hosted) / reverse proxy (self-host) |
...
front of it, and the self-host bundle expects a TLS reverse proxy (or the same
ingress). So no external hop is ever plaintext.
...
Put the control plane behind a TLS reverse proxy (Caddy, nginx, Traefik) that
terminates HTTPS and forwards to `control-plane:8090`. Do **not** publish port
8090 to the public internet directly; expose only the proxy's 443.
