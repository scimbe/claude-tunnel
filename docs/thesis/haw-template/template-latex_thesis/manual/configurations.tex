...
\begin{center}
  \begin {tabular}{ll}
    ...
    \texttt{EMI\_REE} & Bachelor Regenerative Energiesysteme und Energiemanagement \\
    \texttt{EMI\_IE} & Bachelor Information Engineering \\
    ...
    \texttt{EMI\_MaICE} & Master Information and Communication Engineering \\
    \texttt{EMI\_MaMS} & Master Microelectronic Systems \\
  \end{tabular}
\end{center}

\section{Konfiguration im \LaTeX -Code}\label{sec:codechanges}
Für einige Anpassungen müssen direkt eine Änderungen im \LaTeX -Code vorgenommen werden. 

Soll eine Liste von Listings eingefügt werden, so ist die Zeile in der Datei \texttt{thesis.tex} auszukommentieren und das Paket \texttt{listings} in der Paketlist hinzuzufügen.

Soll die Arbeit im einseitigen Layout erstellt werden, so ist in der Datei \texttt{thesis.tex} in der Parameterliste die Option \texttt{twoside}auszukommentieren.
