...
# pow prepended to the client's 12-column csv_row (delay..p99_ms).
HEADER="pow,delay,loss,rate,n,mean_ms,min_ms,max_ms,p50_ms,p95_ms,stddev_ms,ci95_ms,p99_ms"
...
run_condition() {
    local pow="$1" delay="$2" loss="$3" rate="$4"
    echo "sweep: pow=$pow delay=${delay:-none} loss=${loss:-none} rate=${rate:-none} (${ITER} iters)"
    local out row
    out=$(EDGE_POW_DIFFICULTY="$pow" EDGE_DELAY="$delay" EDGE_LOSS="$loss" EDGE_RATE="$rate" \
...
for pow in $POWS; do
    for delay in $DELAYS; do
        for loss in $LOSSES; do
            if [ -z "$RATES" ]; then
                run_condition "$pow" "$delay" "$loss" ""
            else
                for rate in $RATES; do
                    run_condition "$pow" "$delay" "$loss" "$rate"
                done
            fi
        done
    done
